	...
	if(isset($_GET['type']) && isset($_GET['id'])){
		...
		if(is_numeric($_GET['id'])){
			...
			if(in_array($_GET['type'], $allowed)){
				...
				$user_class = new User();
				$post->like_post($_GET['id'],$_GET['type'],$_SESSION['student_userid']);
 
				if($_GET['type'] == "user"){
					$user_class->follow_user($_GET['id'],$_GET['type'],$_SESSION['student_userid']);
		...
		$info = "";
				...
				if(isset($_SESSION['student_userid'])){
					...
					if(is_array($result)){
						...
						if(in_array($_SESSION['student_userid'], $user_ids)){
							$i_liked = true;
						}
					}
